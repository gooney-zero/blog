...
export function fetchGet(config: Omit<IRequestConfig, 'method'>): <T>(body: IObj) => Promise<AxiosResponse<T>> {
  const { url } = config;
  return (body = {}) => {
...
export function fetchPost(config: Omit<IRequestConfig, 'method'>): <T>(body: IObj) => Promise<AxiosResponse<T>> {
  const { url } = config;
  return (body) => {
    return reload({ url, body });
  };
}
export function customRequest(config: IRequestConfig, cleanout = true): <T>(body: any) => Promise<AxiosResponse<T>> {
